...
class TwitterStatusInputTest extends FunSuite with MockitoSugar {
  ...
  test ("The Flink source should correctly be closed and opened") {
    val source = new TwitterStatusSource(consumerToken, accessToken, Seq.empty,  Seq.empty,  Seq.empty,  Seq.empty, false, FilterLevel.None)
  ...
  test ("The tweets should be properly collected") {
    ...
    verify(twitterClient).filterStatuses(any(), any(), any(), any(), any(), any())(argumentCaptor.capture())
